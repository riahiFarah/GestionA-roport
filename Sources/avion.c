...
void suppression_av_donne(Avion** tete, const char* identifiant) {
    ...
    if (courant != NULL) {
        ...
        free(courant);
        printf("L'avion avec l'identifiant %s a été supprimé.\n", identifiant);
    } else {
        printf("Erreur : Avion avec l'identifiant %s non trouvé.\n", identifiant);
    }
...
void creation_liste_avion(Avion** tete) {
    int continuer = 1; // Contrôle de la boucle
    while (continuer == 1) {
        ...
        // Si l'insertion échoue après 3 tentatives
        if (!insertion_reussie) {
            printf("Vous avez épuisé toutes vos tentatives. Avion non ajouté.\n");
            free(nouvel_avion); // Libérer la mémoire de l'avion non ajouté
        }

        // Demander à l'utilisateur s'il souhaite continuer
        printf("Voulez-vous ajouter un autre avion ? (1 pour Oui, 0 pour Non) : ");
        scanf("%d", &continuer); // Si l'utilisateur entre autre chose que 1, la boucle s'arrête
    }
}
...
void ajouter_avion(Avion** tete, Avion* nouvel_avion) {
    if (*tete == NULL) {
        // Si la liste est vide, le nouvel avion devient le premier élément
        *tete = nouvel_avion;
    } else {
        // Sinon, on ajoute le nouvel avion à la fin de la liste
        Avion* temp = *tete;
...
// Fonction pour supprimer les avions d'une compagnie donnée
